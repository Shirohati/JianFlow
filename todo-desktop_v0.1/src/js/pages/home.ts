...
import { renderTaskItem, type TaskItemRenderState } from '../components/sections/task-item';

function taskItemState(t: TaskItem): TaskItemRenderState {
  return {
    isToday: (store.get<string>('currentDate') ?? utils.getTodayStr()) === utils.getTodayStr(),
    stepsOpen: homeOpenSteps.has(t.id),
    stepEditing: homeStepEditing,
    stepAddingId: homeStepAddingId,
  };
}
...
export const homePage = {
  ...
  saveAllEdits(): void {
    ...
    editables.forEach(el => {
      ...
      // Trigger blur to ensure any pending edits are captured
      fieldEl.blur();
    });
    // Now save all edits
    const editables2 = document.querySelectorAll('#page-home [contenteditable="true"]');
  ...
  renderTaskList(tasks: TaskItem[], allTasks: TaskItem[]): void {
    ...
    html += `<div class="home-todo-sec home-todo-sec--primary">
      ...
      <div class="home-todo-sec__list">${pendingPrimary.length > 0 ? pendingPrimary.map(t => renderTaskItem(t, allTasks, categories, taskItemState(t))).join('') : `<div class="home-todo-sec__empty">暂无主要待办</div>`}</div>
    </div>`;
    if (showSecondary) {
      ...
      html += `<div class="home-todo-sec home-todo-sec--secondary${collapsed ? ' home-todo-sec--collapsed' : ''}">
        ...
        <div class="home-todo-sec__list">${pendingSecondary.length > 0 ? pendingSecondary.map(t => renderTaskItem(t, allTasks, categories, taskItemState(t))).join('') : `<div class="home-todo-sec__empty">暂无次要待办</div>`}</div>
      </div>`;
    }
    if (completed.length > 0) {
      html += `<div class="home-completed-header">${icon('check-circle-2', 'size="14"')} 已完成 (${completed.length})</div>`;
      html += completed.map(t => renderTaskItem(t, allTasks, categories, taskItemState(t))).join('');
    }
    el.innerHTML = html;
  },
